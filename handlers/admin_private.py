...
from filtres.filtres_types import IsAdmin
...
Машина состояний для добавления или изменения превью
...
# встаем в состояние отправки фото
@admin_router.message(StateFilter(None), F.text == 'Добавить/Изменить превью')
...
async def add_banner(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    await message.answer("Превью добавлено/изменено")
    await state.clear()


'''

Машина состояний для добавления или изменения товаров
...
# встаем в состояние ожидания ввода name
...
async def category_choice(callback: types.CallbackQuery, state: FSMContext , session: AsyncSession):
    if int(callback.data) in [category.id for category in await orm_get_categories(session)]:
        ...
        await callback.message.answer('Выберите имеющуюся категорию')
        await callback.answer()
...
async def add_image(message: types.Message, state: FSMContext, session: AsyncSession):
    if message.text and message.text == "." and AddProduct.product_for_change:
        ...
        await message.answer("Отправьте корректное фото")
        return
